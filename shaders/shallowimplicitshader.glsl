...
uniform sampler2D x_t1k0; // next timestep, previous iteration solution
...
//    float dv_dt = (h>0) ? g*(dfield_dy.b+dH_dy) -udv_dx - vdv_dy : 0.0;
    ...
    vec4 field_t0 = texture(x_t0, texcoord);

    float u_out = field_t0.r + dt * du_dt;
    float v_out = field_t0.g + dt * dv_dt;
    float h_out = field_t0.b + dt * dh_dt;
...
//    }
    ...
    // output
    x_t1k1 = vec4(u_out, v_out, h_out, 1.0);
